...
class Qwen2VLRewardModelBT(Qwen2VLForConditionalGeneration):
    ...
    def forward_with_embeds(
        ...
        video_grid_thw: Optional[torch.LongTensor] = None,
        rope_deltas: Optional[torch.LongTensor] = None,
        pixel_grad: bool = False,
    ):
        """Like forward(), but returns video_embeds with grad enabled for spatial saliency computation.

        When pixel_grad=False (default): gradient target is video_embeds (after ViT, patch-level).
        When pixel_grad=True: gradient target is pixel_values_videos (before ViT, pixel-level).
            The ViT activations are retained for backprop, costing extra VRAM.

        Returns dict with:
            - logits: [B, output_dim] reward scores
            - video_embeds: the video embedding tensor (with requires_grad=True) for gradient computation
            - pixel_values_videos: (only when pixel_grad=True) pixel tensor with requires_grad=True
            - video_grid_thw: grid dimensions for reshaping spatial maps
            - visual_attn_weights: attention weights from the last layer of visual encoder (if available)
        """
        output_attentions = output_attentions if output_attentions is not None else self.config.output_attentions
        output_hidden_states = (
            output_hidden_states if output_hidden_states is not None else self.config.output_hidden_states
        ...
        # --- Visual encoding with attention hook ---
        visual_attn_weights = []

        def _attn_hook(module, input, output):
            # Qwen2VL visual encoder uses SdpaAttention; grab the hidden states output
            # We register on the last attention block to get the attention pattern
            if isinstance(output, tuple) and len(output) > 1:
                visual_attn_weights.append(output[1])  # attention weights if returned

        # Register hook on the last attention layer of visual encoder
        hook_handle = None
        if hasattr(self.visual, 'blocks') and len(self.visual.blocks) > 0:
            last_block = self.visual.blocks[-1]
            if hasattr(last_block, 'attn'):
                hook_handle = last_block.attn.register_forward_hook(_attn_hook)
        ...
        if inputs_embeds is None:
            ...
            if pixel_values_videos is not None:
                ...
                if pixel_grad:
                    # Pixel-level saliency: enable grad on raw pixels, let it flow through ViT
                    pixel_values_videos = pixel_values_videos.detach().requires_grad_(True)
                    video_embeds = self.visual(pixel_values_videos, grid_thw=video_grid_thw)
                    video_embeds = video_embeds.to(inputs_embeds.device, inputs_embeds.dtype)
                else:
                    # Patch-level saliency (default): detach after ViT, enable grad on embeds
                    video_embeds = self.visual(pixel_values_videos, grid_thw=video_grid_thw)
                    video_embeds = video_embeds.to(inputs_embeds.device, inputs_embeds.dtype)
        ...
        result = {"logits": pooled_logits}
        if video_embeds is not None:
            result["video_embeds"] = video_embeds
        if pixel_grad and pixel_values_videos is not None:
            result["pixel_values_videos"] = pixel_values_videos
        if video_grid_thw is not None:
            result["video_grid_thw"] = video_grid_thw
        if visual_attn_weights:
            result["visual_attn_weights"] = visual_attn_weights[-1]
        return result
