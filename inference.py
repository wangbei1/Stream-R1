...
if args.checkpoint_path:
    ...
    # Strip FSDP wrapper prefixes left by fsdp_state_dict()
    strip_prefixes = ("_fsdp_wrapped_module.", "_checkpoint_wrapped_module.", "_orig_mod.")
    cleaned = {}
    ...
    if idx < num_prompts:
        for seed_idx in range(args.num_samples):
            ...
            if args.save_with_index:
                filename = f'{idx:04d}_seed{actual_seed}.mp4'
            else:
                safe_prompt = prompt[:50].replace('/', '_').replace('\\', '_')
                if args.num_samples > 1:
                    filename = f'{safe_prompt}_seed{actual_seed}.mp4'
                else:
